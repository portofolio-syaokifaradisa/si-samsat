          ...
          <div class="card">
            <div class="card-header">
              ...
              <div class="card-header-form">
                <a href="{{ route('admin.mutasi.print', ['type' => $type]) }}" class="btn btn-primary" target="_blank">
                  Cetak Pengajuan 
                </a>
              </div>
            </div>
                      ...
                      @foreach ($orders as $index => $data)
                        <tr>
                              ...
                              <div class="btn-group dropright px-0 pr-2">
                                ...
                                <div class="dropdown-menu dropright">
                                  @if($data->status == "TERKIRIM")
                                    <a class="dropdown-item has-icon" href="{{ route('admin.mutasi.review', ['id' => $data->id, 'type' => $type]) }}">
                                      Terima
                                    </a>
                                    <a class="dropdown-item has-icon" href="{{ route('admin.mutasi.reject', ['id' => $data->id, 'type' => $type]) }}">
                                      Tolak
                                    </a>
                                  @elseif ($data->status == "DITERIMA")
                                    <form action="{{ route('admin.mutasi.finishing', ['id' => $data->id, 'type' => $type]) }}" method="post" id="finishing-form-{{ $index }}">
                                      @csrf
                                      @method('PUT')
                          ...
                          <td class="text-center align-middle">
                            ...
                            @if ($data->surat_fiskal)
                              <a href="{{ $data->surat_fiskal_path }}" target="_blank">
                                Surat Fiskal
                              </a>
                            @endif
                          </td>
