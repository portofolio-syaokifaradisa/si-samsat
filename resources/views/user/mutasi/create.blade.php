...
<section class="section">
    <div class="section-header">
      <h1>Pengajuan Mutasi {{ ucwords($type) }}</h1>
    </div>
      ...
      <div class="row">
        <div class="col-12">
          ...
          <div class="card">
            ...
            <form action="@if(isset($order)) {{ route('mutasi.update', ['id' => $order->id, 'type' => $type]) }} @else {{ route('mutasi.store', ['type' => $type]) }} @endif" method="POST" enctype='multipart/form-data'>
                @csrf
                @if(isset($order))
                    @method('PUT')
                @endif
                ...
                <div class="card-body px-4 py-0">
                    ...
                    <div class="row">
                        <div class="form-group col-6">
                            <label><b>Surat Rekomendasi Polda</b></label>
                            <div class="custom-file">
                            ...
                            @if (isset($order->polda_recommendation))
                                <small class="mt-2">
                                    File Surat Rekomendasi Polda anda sebelumnya dapat dilihat <a href="{{ $order->polda_recommendation_path }}" target="_blank">disini</a>
                                </small>
                            @endif
                        </div>
                        <div class="form-group col-6">
                        ...
                        <div class="form-group col-6">
                            ...
                            <div class="custom-file">
                                <input type="file" name="notice_pajak" class="custom-file-input" id="uploaded-file-form-notice_pajak" @if(!isset($order)) required @endif>
                                <label class="custom-file-label" for="uploaded-file-form" id="uploaded-file-label-notice_pajak">Choose file</label>
                            </div>
                        ...
                        <div class="form-group col-6">
                            ...
                            @if (isset($order->bpkb3))
                                <small class="mt-2">
                                    File BPKB Halaman 3 anda sebelumnya dapat dilihat <a href="{{ $order->stnk3_path }}" target="_blank">disini</a>
                                </small>
                            @endif
                        </div>
                        <div class="form-group col-6">
                        ...
                        @if ($type == "masuk")
                            <div class="form-group col-6">
                                <label><b>Surat Fiskal</b></label>
                                <div class="custom-file">
                                    <input type="file" name="surat_fiskal" class="custom-file-input" id="uploaded-file-form-surat_fiskal" @if(!isset($order)) required @endif>
                                    <label class="custom-file-label" for="uploaded-file-form" id="uploaded-file-label-surat_fiskal">Choose file</label>
                                </div>
                                @if (isset($order->surat_fiskal))
                                    <small class="mt-2">
                                        File Surat Fiskal anda sebelumnya dapat dilihat <a href="{{ $order->surat_fiskal_path }}" target="_blank">disini</a>
                                    </small>
                                @endif
                            </div>
                        @endif
...
@section('js-extends')
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function(){
            ...
            document.getElementById('uploaded-file-form-notice_pajak').addEventListener('change', setFileNameUploaded);
            ...
            var fiskalForm = document.getElementById('uploaded-file-form-surat_fiskal');
            if(fiskalForm){
                fiskalForm.addEventListener('change', setFileNameUploaded)
            }
        });
    </script>
@endsection